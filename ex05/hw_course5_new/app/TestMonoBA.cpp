...
int main() {
    // 准备数据
    vector<Frame> cameras; // for all poses
    vector<Eigen::Vector3d> points; // for all landmarks
    GetSimDataInWordFrame(cameras, points);
    Eigen::Quaterniond qic(1, 0, 0, 0); // transformation between IMU and camera
...
for (auto weight : weights){ // try all the weights
    ...
    // gauge fixation, set first 2 frames Hessian = 0
    //    if(i < 2)
    //        vertexCam->SetFixed();
...
//        double inverse_depth = 1. / Pc.z();
    ...
    //compute sum of errors from inverse depth of landmarks
    ...
    result_csv << "weight is " << weight << "," << sumErrorlandmark*1e6 << "*10^-6," << problem.totalSolveCost << "ms\n";
}
