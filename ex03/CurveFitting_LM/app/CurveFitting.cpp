...
{
    #ifdef USE_OPENMP
    std::cout << "use OpenMP!" << endl;
    #endif
    double a=1, b=2, c=1;         // 真实参数值
    int N = 100;                          // 数据点
...
//        double y = std::exp( a*x*x + b*x + c );
    ...
    if (myfile.is_open()){
        for (auto item : problem.result){
            myfile << item << ", ";
        }
        myfile.close();
    }
    Eigen::Matrix3i testMatrix = Eigen::Matrix3i::Random();
    cout <<"\n" << testMatrix << endl;
    cout << testMatrix.diagonal() << endl;
    cout << Eigen::Matrix3i(testMatrix.diagonal().asDiagonal()) << endl;
    //// wrong way to use asDiagonal with a Matrix (must be Vector instead)
    // auto resMat = testMatrix.asDiagonal();
    // Eigen::Vector3i identityVec ={1,1,1};
    // cout << identityVec << endl;
    // cout << identityVec.transpose() * resMat<< endl;
