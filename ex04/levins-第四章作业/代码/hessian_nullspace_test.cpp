...
int main()
{
    ...
    for(int j = 0; j < featureNums; ++j)
    {
        ...
        for (int i = 0; i < poseNums; ++i) {
            ...
            //     | lp ll |
            // compute block lp
            H.block(6*poseNums + 3*j, 6*i, 3, 6) += jacobian_Pj.transpose() * jacobian_Ti;
            // compute block pl
            H.block(6*i, 6*poseNums + 3*j, 6, 3) += jacobian_Ti.transpose() * jacobian_Pj;
            // compute block ll
...
//    std::cout << H.block(6*poseNums, 0, 3*featureNums/5, 3*featureNums/5) << std::endl;
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> saes(H);
    std::cout << saes.eigenvalues() <<std::endl;// same result as SVD,because H is symmeric, semi-positive definite 
