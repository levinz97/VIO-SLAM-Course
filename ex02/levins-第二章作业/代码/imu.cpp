...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    bool use_Euler_method = false;
    Eigen::Quaterniond dq;  // update for quaternion
    Eigen::Vector3d acc_w;  // update for velocity
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        if(use_Euler_method){
            ...
            // use Midpoint method
            MotionData last_imupose = imudata[i - 1];// use last pose for midpoint method instead of future pose

            Eigen::Vector3d dtheta_half = (imupose.imu_gyro + last_imupose.imu_gyro) / 2.0 * dt / 2.0;
            dq.w() = 1;
            dq.x() = dtheta_half.x();
            dq.y() = dtheta_half.y();
            dq.z() = dtheta_half.z();
            ...
            acc_w = Qwb * (imupose.imu_acc + last_imupose.imu_acc) / 2.0 + gw;
        }

        // differences between Euler and Midpoint only in the calculation of acc_w and dq
        // update values in the same way 
        Qwb = Qwb * dq;
        Pwb = Pwb + Vw * dt + 0.5 * pow(dt, 2) * acc_w;
        Vw = Vw + acc_w * dt;
